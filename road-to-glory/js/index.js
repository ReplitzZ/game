...
var boreDOM = {
				                ...
				                if (boreDOM.testCollision(bullet.position(), bulletSize, enemy.position(), enemySize, enemy)){	
				                	...
				                	if(enemy.position().top + 25 > 0){ // Make sure that the object is visible
										...
										if(boreDOM.count.number == 2000){
											$('#ship').css({'background-image':'url(http://i278.photobucket.com/albums/kk105/zymixis/swattds.png)'});
	...
	triggerExplosion: function(topPosition, lefPosition, playerShip){
		...
		$('<div class="'+explosionClass+'"><img src="https://dl.dropboxusercontent.com/u/82861970/BoreDOM_IMG/explosion.gif"></div>').appendTo('body').each(function(){
			var explosion = $(this);
	...
	decreaseLife: function() {
    	...
    	if($('.life-bar .inside-bar').height() <= 0 ){
    		setTimeout(function(){
	       		...
	       		if(confirm('You were defeated by the super soldiers and doomed to be their experiment for the rest of eternity.')){
